...
class CheckoutController extends Controller
{
    ...
    public function index()
    {
        ...
        // Handle Selected IDs (Merge request with session)
        $reqSelected = collect((array) request('selected'))->map(fn($v) => (int)$v)->filter()->values();
        if ($reqSelected->isNotEmpty()) {
            session(['checkout_selected_ids' => $reqSelected]);
        }
        $selectedIds = session('checkout_selected_ids', collect([]));
        
        // Handle Shipping Method
        $shippingMethod = session('checkout_shipping_method', 'regular');
        ...
        // If items empty but we had selected IDs, maybe they were deleted.
        // If absolutely no items, redirect to cart.
        if ($items->isEmpty()) {
             if ($selectedIds->isNotEmpty()) {
                 session()->forget('checkout_selected_ids');
             }
             // Only redirect if we really have no items to checkout
             // return redirect()->route('cart')->with('error', 'Tidak ada item untuk diproses');
        }
        ...
        if ($voucherCode) {
            $voucher = Voucher::active()->where('code', $voucherCode)->first();
            if ($voucher) {
                if ($subtotal >= $voucher->min_purchase) {
                    ...
                    $voucherMessage = "Voucher applied: " . $voucher->code;
                } else {
                    $voucherMessage = "Min purchase not met";
                }
            } else {
                 // Hardcoded fallbacks
                 if ($voucherCode === 'PROMO10') {
                     $voucherDiscount = floor($subtotal * 0.10);
                     $voucherMessage = "Promo 10% applied";
                 } elseif ($voucherCode === 'ONGKIRFREE') {
                     $shippingDiscount = $shipping;
                     $voucherMessage = "Free Shipping applied";
                 } elseif ($voucherCode === 'PROMO20K') {
                     $voucherDiscount = 20000;
                     $voucherMessage = "Discount 20k applied";
                 } elseif ($voucherCode === 'SHIP5K') {
                     $shippingDiscount = min(5000, $shipping);
                     $voucherMessage = "Shipping Discount 5k applied";
                 } else {
        ...
        // Auto apply shipping discount if > 100k
        ...
        return view('checkout.index', compact(
            ...
            'shippingMethod', 'voucherCode', 'voucherDiscount', 
            'shippingDiscount', 'netShipping', 'serviceFee', 'voucherMessage',
            'selectedIds'
        ));
    }
    ...
    public function update(Request $request)
    {
        if ($request->has('shipping_method')) {
            session(['checkout_shipping_method' => $request->shipping_method]);
        }
        
        // Handle Voucher
        if ($request->has('apply_voucher')) {
            // User clicked a voucher chip
            session(['checkout_voucher_code' => strtoupper(trim($request->apply_voucher))]);
        } elseif ($request->has('voucher_code')) {
            // User manually entered code
            session(['checkout_voucher_code' => strtoupper(trim($request->voucher_code))]);
        }
        
        return redirect()->route('checkout.index')->withInput()->with('success', 'Kalkulasi diperbarui');
    }
